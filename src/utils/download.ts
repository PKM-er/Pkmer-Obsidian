...
export async function downloadAndInstallPlugins(pluginId: string, token: string) {
    ...
    const downloadUrl = await api.getDownloadUrl(pluginId)

    try {
        await downloadAndUnzip(downloadUrl, pluginId);
        new Notice(`下载${pluginId}并安装成功！`)
    } catch (error) {
        new Notice(`下载${pluginId}并安装失败！`)
    }
}

async function downloadAndUnzip(url: string, id: string) {
    try {
